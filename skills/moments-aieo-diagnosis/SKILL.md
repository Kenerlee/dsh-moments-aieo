...
### 文件保存位置
```
/Users/kadenliu/AIRetail/Z_GEO/01_diagnosis/
```
...
# 创建诊断报告
touch "/Users/kadenliu/AIRetail/Z_GEO/01_diagnosis/{品牌名}_GEO诊断报告_${DATE}.md"

# 创建问题库初稿
touch "/Users/kadenliu/AIRetail/Z_GEO/01_diagnosis/{品牌名}_问题库_${DATE}.md"
```
...
**重要**: 使用 Playwright MCP 工具进行真实的 AI 平台可见性测试。
...
参考案例（数字校园平台、创业培训、消费品牌）保存在本地工作目录，不随本仓库分发。
...
诊断报告必须以这一节收尾。给判断，不给推销——写清楚这个区间的典型卡点，以及跨到下一区间**缺的到底是什么**。
...
交付给客户时如需去除署名，删掉这一行即可——本 Skill 不做任何强制校验。
