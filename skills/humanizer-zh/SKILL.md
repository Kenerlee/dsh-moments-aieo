...
name: humanizer-zh
description: |
  去除文本中的 AI 生成痕迹，使其听起来更自然、更像人类书写。使用场景：(1) 用户要求"人性化"或"去除 AI 味道"处理文本；(2) 用户要求编辑文本使其"更自然"、"更像人写的"；(3) 用户要求检查或修复 AI 写作特征。检测并修复以下模式：夸大的象征意义、宣传性语言、以 -ing 结尾的肤浅分析、模糊的归因、破折号过度使用、三段式法则、AI 词汇、否定式排比、过多的连接性短语。
---
...
**问题：** ChatGPT 使用弯引号（""）而不是直引号（""）。中文通常使用中文引号（「」或""）。
