...
metadata:
  ...
  derived-from: content-creator by Alireza Rezvani (MIT)
  iterated-by: Moments (https://moments.top)
  category: marketing
...
### ⚠️ MANDATORY: Diagnosis & Positioning Check (Always First)

Before ANY content creation workflow, you MUST check the results of diagnosis and positioning:

1. **Review Diagnosis Results**
   - Check the current site/brand diagnosis output (e.g., SEO audit, brand health check, competitor analysis)
   - Identify all flagged issues, gaps, and opportunities
   - Confirm the positioning strategy is defined and up to date

2. **Validate Positioning Alignment**
   - Ensure content direction aligns with the established positioning
   - Verify target audience segments are clearly defined
   - Confirm key messaging pillars match the diagnosis findings

3. **Document Findings**
   - List all issues that need to be addressed through content
   - Rank issues by priority (critical → high → medium → low)
   - These findings feed directly into the Content Calendar (high-priority issues first)

> **Do NOT proceed to any content creation workflow until diagnosis and positioning results have been reviewed and validated.**
...
1. **Prioritize Issues & High-Priority Topics First**
   - Pull all issues identified from the Diagnosis & Positioning Check
   - Rank content topics by priority: issues and gaps flagged in diagnosis take precedence
   - High-priority topics (fixing SEO gaps, addressing audience pain points, correcting positioning weaknesses) MUST be scheduled before general/routine content
   - Map each issue to a specific content piece with a target publish date

2. **Monthly Planning**
   - Copy `assets/content_calendar_template.md`
   - Set monthly goals and KPIs
   - Slot high-priority issue-driven content into the first weeks of the month
   - Identify key campaigns/themes for remaining slots

3. **Weekly Distribution**
   - Follow 40/25/25/10 content pillar ratio
   - Ensure at least 1-2 issue-addressing pieces per week until all critical issues are covered
...
When deploying content to a Netlify server, you MUST use the **access token method only**. No other deployment method is permitted.

### Netlify Access Token Deployment

1. **Ask the User for Credentials**
   - Request the Netlify **Personal Access Token**
   - Request the **Site ID** (found in Site Settings → General → Site ID)
   - Do NOT attempt to use `netlify login`, OAuth, or browser-based auth flows

2. **Deploy Using the Access Token**
   ```bash
   # Deploy using access token (the ONLY permitted method)
   NETLIFY_AUTH_TOKEN=<user-provided-token> netlify deploy --site <site-id> --prod --dir <build-dir>
   ```
   - If the `netlify` CLI is not available, use the Netlify API directly:
   ```bash
   curl -H "Authorization: Bearer <user-provided-token>" \
        -H "Content-Type: application/zip" \
        --data-binary @deploy.zip \
        "https://api.netlify.com/api/v1/sites/<site-id>/deploys"
   ```

3. **Never Store Tokens**
   - Do not write tokens to config files, `.env` files, or any persisted location
   - Use tokens only in the current session via environment variables

> **Any other Netlify deployment method (OAuth, browser login, `netlify login`) is NOT allowed.**

---

## Post-Deployment: Server Verification

After deploying content, you MUST check the live server to confirm issues are fixed:

1. **Verify Deployment Success**
   - Check the Netlify deploy log output for success status
   - Confirm the deploy URL is accessible

2. **Check the Live Site**
   - Fetch the deployed page and verify the content renders correctly:
   ```bash
   curl -s -o /dev/null -w "%{http_code}" https://<your-site>.netlify.app/
   ```
   - Confirm HTTP 200 status code
   - Spot-check key pages that were updated

3. **Validate Issue Fixes**
   - For each issue identified in the Diagnosis & Positioning Check, verify the corresponding content fix is live
   - Check SEO elements (meta tags, headings, keyword placement) on the deployed pages
   - Confirm no broken links or missing assets

4. **Report Results**
   - Summarize which issues have been verified as fixed on the live server
   - Flag any issues that remain unresolved
   - Document any new issues discovered during verification

> **Do NOT mark content tasks as complete until the live server has been checked and issues are confirmed fixed.**
